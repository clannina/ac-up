...
import { Scale, Droplet, Flame, Activity, Flag, HeartPulse, CalendarHeart, Loader2, Check, Pencil, Trash2, X, Plus } from "lucide-react";
...
export default function Salute() {
  ...
  const saveWeight = async () => {
    ...
    if (!weight_kg || !supabase || !session?.user?.id) return;
  ...
  const currentWeight = weightHistory[0]?.weight_kg ?? 98;
  const weightDelta =
    weightHistory.length >= 2 ? (weightHistory[0].weight_kg - weightHistory[1].weight_kg).toFixed(1) : null;
  ...
  const span = 30;
  const distance = currentWeight - stats.target;
    ...
    <Page>
      ...
      <div className={`${GLASS} rounded-[28px] p-7 mt-8`}>
        ...
        <div className="flex items-end justify-between mb-5">
          <div>
            <span className="font-mono-num text-5xl font-bold text-white">{currentWeight}</span>
      ...
      <div className={`${GLASS} rounded-[28px] p-7 mt-6`}>
        <div className="grid grid-cols-3 gap-4">
          <Ring value={currentWeight} max={currentWeight + 5} icon={Scale} label="Peso" sub="kg" />
          <Ring value={water} max={8} icon={Droplet} label="Acqua" sub="/ 8" />
          <Ring value={stats.calories} max={stats.calorieTarget} icon={Flame} label="Calorie" sub={`/ ${stats.calorieTarget}`} />
        </div>

        <div className="grid grid-cols-2 gap-4 mt-6 pt-6" style={{ borderTop: "1px solid rgba(255,255,255,0.15)" }}>
          <div>
            <div className="flex gap-2">
              <input
                ...
                className="w-full rounded-xl px-3 py-2 outline-none font-mono-num placeholder-white/50 text-sm"
                style={{ border: "1px solid rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.15)", color: "white" }}
              />
              <button
                onClick={saveWeight}
                disabled={savingWeight}
                className="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center bg-white"
                style={{ color: T.forest }}
              >
                {savingWeight ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />}
              </button>
            </div>
            {weightSaved && (
              <span className="flex items-center gap-1 text-xs text-white mt-1.5">
          ...
          <button
            onClick={addWater}
            className="flex items-center justify-center gap-1.5 rounded-xl py-2 text-sm font-semibold text-white h-fit"
            style={{ border: "1px solid rgba(255,255,255,0.3)" }}
          >
            <Plus size={14} /> Bicchiere d'acqua
          </button>
        </div>

        <p className="text-xs text-white/40 mt-4">
          Le calorie non sono ancora collegate a un diario giornaliero reale.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mt-6">
        <div className={`${GLASS} rounded-[28px] p-6`}>
          ...
          <div className="w-full rounded-full h-2 mb-6 bg-white/20">
            <div className="h-2 rounded-full bg-white" style={{ width: `${Math.round((stats.target / currentWeight) * 100)}%` }} />
          </div>

          <p className="mb-2 text-sm text-white/70">Acqua</p>
          <div className="w-full rounded-full h-2 mb-6 bg-white/20">
            <div className="h-2 rounded-full bg-white" style={{ width: `${Math.round((water / 8) * 100)}%` }} />
          </div>
